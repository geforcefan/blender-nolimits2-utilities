...
from .nolimits_terrain_object import add_object, build_terrain_mesh, build_water_mesh
...
class NOLIMITS2_OT_import(bpy.types.Operator, ImportHelper):
    ...
    def import_park(self, context):
        ...
        if self.import_terrain and park.terrain is not None:
            ...
            created.append(add_object(context, build_terrain_mesh(f"{park_name} Terrain", park.terrain)))
            created.append(add_object(context, build_water_mesh(f"{park_name} Water", *park.terrain.size())))

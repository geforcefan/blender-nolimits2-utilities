...
class BlenderSplineMapping:
    def __init__(self, curve, closed):
        ...
        for index in range(count):
            ...
            self.co[index * 3:index * 3 + 3] = z_up(position)
            self.handle_left[index * 3:index * 3 + 3] = z_up(
                (position[0] - incoming[0] / 3.0, position[1] - incoming[1] / 3.0, position[2] - incoming[2] / 3.0))
            self.handle_right[index * 3:index * 3 + 3] = z_up(
                (position[0] + outgoing[0] / 3.0, position[1] + outgoing[1] / 3.0, position[2] + outgoing[2] / 3.0))
            tangent = vec3_normalize(z_up(outgoing if vec3_length(outgoing) > 0.0 else incoming))
            right = z_up(quat_rotate(node_at(index).orientation, vec3(-1.0, 0.0, 0.0)))
